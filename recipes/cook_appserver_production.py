...
create_recipe = [
    ...
    # setup env variables
    {"action": "export_envs", "params": "", "message": "Exporting variables"},
    ...
    {"action": "sudo", "params": "apt-get install -qq libjpeg62 libjpeg62-dev zlib1g-dev"},
    {"action": "sudo", "params": "sudo ln -s /usr/lib/x86_64-linux-gnu/libjpeg.so /usr/lib"},
...
production_deploy_recipe = [
    # First command as regular user
    {"action": "run", "params": "whoami"},

    {"action": "run", "params": "cd %(APPS_DIR)s/%(PROJECT_NAME)s; git checkout %(GITBUH_STAGING_BRANCH)s"},

    {"action": "run", "params": "cd %(APPS_DIR)s/%(PROJECT_NAME)s; git pull"},

    {"action": "run", "params": "cd %(APPS_DIR)s/%(PROJECT_NAME)s; git checkout %(GITHUB_BRANCH)s"},

    {"action": "run", "params": "cd %(APPS_DIR)s/%(PROJECT_NAME)s; git merge %(GITBUH_STAGING_BRANCH)s"},

    {"action": "run", "params": "cd %(APPS_DIR)s/%(PROJECT_NAME)s; git push"},
